...
consumes:
  - artifact: inception/requirements/requirements.md
    required: true
  - artifact: inception/requirements/requirement-verification-questions.md
    required: false
...
### 3.0 Start from the Active Scope Baseline

The active scope (selected and confirmed during Requirements Analysis, recorded in `aidlc-state.md`) provides the starting plan. The matrix below is generated from stage frontmatter — find the column for the active scope:

<!-- BEGIN GENERATED: scope-matrix | do not hand-edit — regenerated by scripts/generate.py from stage frontmatter (see references/common/stage-contract.md) -->
| Phase | Stage | classic | bugfix | express | infra | refactor | security-patch |
|---|---|---|---|---|---|---|---|
...
| 🔵 Inception | Application Design | CONDITIONAL | SKIP | SKIP | SKIP | SKIP | SKIP |
| 🔵 Inception | Units Generation | CONDITIONAL | SKIP | SKIP | SKIP | SKIP | SKIP |
| 🟢 Construction | Functional Design | CONDITIONAL | SKIP | SKIP | SKIP | EXECUTE | SKIP |
| 🟢 Construction | NFR Requirements | CONDITIONAL | SKIP | SKIP | EXECUTE | SKIP | EXECUTE |
| 🟢 Construction | NFR Design | CONDITIONAL | SKIP | SKIP | EXECUTE | SKIP | SKIP |
| 🟢 Construction | Infrastructure Design | CONDITIONAL | SKIP | SKIP | EXECUTE | SKIP | SKIP |
...
The state file `aidlc-docs/aidlc-state.md` is created deterministically by the
engine during Workspace Detection:

```cmd
python <skill>/scripts/engine.py init
```

Do **not** create it here, and never hand-edit its Stage Progress, Current
Status, or State Digest sections — those are the engine's exclusive territory.

After the plan is approved (Steps 9/10), record the plan decisions in the
`## Execution Plan Summary` section of `aidlc-docs/aidlc-state.md`. That section
is the model's territory, but the engine reads it — so the format is strict:

```markdown
## Execution Plan Summary
- **Scope**: <active scope name>
- **Depth**: <minimal|standard|comprehensive>
- **Stages to Execute**: slug1, slug2, ...
- **Stages to Skip**: slug1 (reason), slug2 (reason), ...
```
...
```cmd
python <skill>/scripts/engine.py report --stage workflow-planning --result approved
```

Do not run `report` before the plan is approved.
